"""
Script to calculate ROUGE and BLEU scores for summaries generated by models for evaluation or development datasets
...
Or just question driven BART and Pointer Generator
...
Or compute wilcoxon b/w hardcoded models
...
def evaluate():
    ...
    #focus on the question summ task
            ...
            for summ_type in ['singleAbstractive', 'singleExtractive']:
                for q in ["with_question", "without_question"]:
                    ...
                    datasets = {
                        ...
                        #'sc_k10+bart_{q}_{t}'.format(q=q, t=summ_type): "data/bart/chiqa_eval/bart_chiqa_{q}_{l}_{t}_sent_class_binary_topk10.json".format(q=q, l=summ_task, t=summ_type),
                        'Pointer Generator-{q}-{t}'.format(q=q, t=summ_type): "data/pointer_generator/chiqa_eval/pointergen_chiqa_bioasq_abs2summ_{q}_{l}_{t}.json".format(q=q, l=summ_task, t=summ_type),
                        #'sc_k10+pg_{q}_{t}'.format(q=q, t=summ_type): "data/pointer_generator/chiqa_eval/pointergen_chiqa_bioasq_abs2summ_{q}_sent_class_10_{l}_{t}.json".format(q=q, l=summ_task, t=summ_type),
                    }
                    ...
                    for eval_set in data:
                        questions = data[eval_set][2]
                        # Save rouge2 per sample to excel for a BART task
                        if summ_type == "singleAbstractive" and summ_task == "page2answer" and "pointer_generator" in eval_set:
                        ...
                        if args.calculate_wilcoxon:
                            #if summ_type == "singleAbstractive" and summ_task == "section2answer" and "BART" in eval_set:
                            if "BART" in eval_set:
                                rouge_scores = []
                                tokenized_gold = tokenize(data[eval_set][0])
                                tokenized_guess = tokenize(data[eval_set][1])
