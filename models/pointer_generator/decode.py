...
import json
import pyrouge
# Using py-rouge, the pure python rouge evaluator 
#import rouge
import util
import logging
import numpy as np
...
class BeamSearchDecoder(object):
  ...
  def __init__(self, model, batcher, vocab):
    ...
    self._generated_answers = {"question": [], "ref_summary": [], "gen_summary": []}

    # For running decode for answer summarization, I have modified this to load the best model from the eval directory 
    ckpt_path = util.load_ckpt(self._saver, self._sess)
    #ckpt_path = util.load_ckpt(self._saver, self._sess, "eval")
    ...
    while True:
      ...
      if batch is None: # finished decoding dataset in single_pass mode
        ...
        if FLAGS.eval_type == "medsumm":
          tf.logging.info("Writing generated answer summaries to file...")
          with open(FLAGS.generated_data_file, "w", encoding="utf-8") as f:
      ...
      if FLAGS.single_pass:
        if FLAGS.eval_type == "medsumm":
          ...
          self.write_data_for_medsumm_eval(original_abstract_sents, decoded_words, question, counter) # write ref summary and decoded summary to file, for later evaluation.
  ...
  def write_data_for_medsumm_eval(self, original_abstract_sents, decoded_words, question, counter):
    """
    ...
    while len(decoded_words) > 0:
      ...
      decoded_sents += (" " + ' '.join(sent))
    abstract_sents = ". ".join(original_abstract_sents)

    self._generated_answers['question'].append(question) 
    self._generated_answers['ref_summary'].append(abstract_sents) 
    self._generated_answers['gen_summary'].append(decoded_sents) 
...
def get_decode_dir_name(ckpt_name):
  ...
  elif "summ" in FLAGS.data_path: dataset = "test"
  else: raise ValueError("FLAGS.data_path %s should contain one of train, val, test, or summ" % (FLAGS.data_path))
