...
def asumm_example_generator(data_path, single_pass, question_driven, tag_sentences):
  """Generates tf.Examples from data files for answer summarization. Coppied from example_generator.
  ...
  Returns data until iterated to the end of the file
  """
  while True:
    with open(data_path, "r", encoding="utf-8") as f:
      asumm_data = json.load(f)
      keys = list(asumm_data.keys())
      random.shuffle(keys)
      for key in keys:
        summary = asumm_data[key]['summary']
        articles = asumm_data[key]['articles']
        question = asumm_data[key]['question']
        # Only add question or tag sentences during inference, not during training
        if tag_sentences and single_pass: 
          split_summ = summary.split(".")
          summary = ' '.join(["%s %s %s" % (SENTENCE_START, sent, SENTENCE_END) for sent in split_summ])
        if question_driven and single_pass:
          articles = question + " [QUESTION?] " + articles
        yield (summary, articles, question)
      if single_pass:
        print("example_generator completed reading all datafiles. No more data.")
        break
