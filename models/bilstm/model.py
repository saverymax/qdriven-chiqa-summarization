...
class SentenceClassificationModel():
    ...
    def __init__(self, vocab_size, batch_size, hidden_dim, dropout, max_tok_q, max_sentences, max_tok_sent, model_image_path):
        """
        Initiate the mode
        """
    ...
    def build_simple_model(self):
        """
        Construct graph of simpler model for testing
        """
        question_input = tf.keras.Input(shape=(self.max_tok_q, ), name='q_input')
        x1 = layers.Embedding(input_dim=self.vocab_size, output_dim=self.hidden_dim, mask_zero=True)(question_input)
        x1 = layers.Bidirectional(layers.LSTM(self.hidden_dim), input_shape=(self.max_tok_q, self.hidden_dim),  name='q_bilstm')(x1)

        sent_output = layers.Dense(1, activation='sigmoid', name='sent_output')(x1)
        model = tf.keras.Model(inputs=question_input, outputs=sent_output)
        model.summary()
    ...
    def build_model(self):
        """
        Construct the graph of the model mapping to rouge scores
        """
        question_input = tf.keras.Input(shape=(self.max_tok_q, ), name='q_input')
        ...
        sent_output = layers.Dense(1, activation='sigmoid', name='sent_output', kernel_regularizer=tf.keras.regularizers.l2(0.01))(x4)
        # What is the difference between this and the previous
        #sent_output = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(1, activation='sigmoid'), name='sent_output')(x4)
        ...
        tf.keras.utils.plot_model(model, self.model_image_path)
        return model
