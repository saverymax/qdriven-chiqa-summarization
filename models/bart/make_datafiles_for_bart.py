"""
This is the make_datafiles script from the pointgenerator cnn-dailymail repo
https://github.com/abisee/cnn-dailymail/blob/b15ad0a2db0d407a84b8ca9b5731e1f1c4bd24b9/make_datafiles.py#L235
but with modifications made here
https://gist.github.com/zhaoguangxiang/45bf39c528cf7fb7853bffba7fe57c7e
The script now saves the cnn dailymail files as test.source and test.target, etc

It requires python 2 to run. Run the activate_py2_env.sh to activate.
"""
...
all_train_urls = "/data/saveryme/asumm/asumm_data/cnn-dailymail/url_lists/all_train.txt"
all_val_urls = "/data/saveryme/asumm/asumm_data/cnn-dailymail/url_lists/all_val.txt"
all_test_urls = "/data/saveryme/asumm/asumm_data/cnn-dailymail/url_lists/all_test.txt"

# cnn_tokenized_stories_dir = "cnn_stories_tokenized"
# dm_tokenized_stories_dir = "dm_stories_tokenized"
cnn_tokenized_stories_dir = "/data/saveryme/asumm/asumm_data/cnn_dm/cnn/stories"
dm_tokenized_stories_dir = "/data/saveryme/asumm/asumm_data/cnn_dm/dailymail/stories"
# finished_files_dir = "finished_files"
finished_files_dir = "cnn_dm_finished_files"
...
def write_to_bin(url_file, out_file, makevocab=False):
  ...
  with open(out_file, 'wb') as writer:
    for idx,s in enumerate(story_fnames):
      ...
      if os.path.isfile(os.path.join(cnn_tokenized_stories_dir, s)) and article[:5] == '(CNN)':
        article = article[5:]

      article.decode('utf-8','ignore').encode('utf-8')
      if '\r' in article:
        cnt_r += 1
        article.replace("\r", " ")
        print "there is a line contains r"
        print cnt_r
      if '\n' in article:
        cnt_n += 1
        article.replace("\n", " ")
        print "there is a line contains n"
        print cnt_n
      article = ' '.join(article.split())
      with open(out_file+'.source', mode='a+') as src:
        src.write(article+'\n')
      with codecs.open(out_file+'.target', mode='a+') as tgt:
        tgt.write(abstract+'\n')
      # Write to tf.Example
...
if __name__ == '__main__':
  ...
  write_to_bin(all_train_urls, os.path.join(finished_files_dir, "train"), makevocab=True)
